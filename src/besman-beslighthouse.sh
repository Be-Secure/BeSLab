...
function __besman_install_beslighthouse()
{
   ...
   source nvm_install.sh | __beslab_log
   
   source ~/.bashrc | __beslab_log
   ...
   cd $beslight_path 
   curl --silent -LJO https://github.com/Be-Secure/BeSLighthouse/archive/refs/tags/${beslight_ver}.tar.gz | __beslab_log
   ...
   if [ -f $gitlab_user_data_file_path ];then
    ...
    sed -i '/"activeTool"/c\"activeTool": "gitlab"' $beslighthouse_config_path  | __beslab_log
    sed -i "/\"namespace\"/c\"namespace\": \"$GITUSER\"," $beslighthouse_config_path | __beslab_log
    sed -i "/\"token\"/c\"token\": \"$GITUSERTOKEN\"" $beslighthouse_config_path  | __beslab_log
    myip="$(dig +short myip.opendns.com @resolver1.opendns.com)"
    sed -i "/\"apiUrl\"/c\"apiUrl\": \"http://$myip:5000\"," $beslighthouse_config_path  | __beslab_log
    sed -i "/\"gitLabUrl\"/c\"gitLabUrl\": \"http://$myip\"," $beslighthouse_config_path  | __beslab_log
